...
import Alarm, { removeAlarm, scheduleAlarm, updateAlarm } from '../components/alarm';
...
export default function ({ route, navigation }) {
  ...
  async function onSave() {
    if (mode === 'EDIT') {
      alarm.active = true;
      await Promise.all(updateAlarm(alarm))
    }
    if (mode === 'CREATE') {
      await Promise.all(scheduleAlarm(alarm))
    }
    navigation.goBack();
  }
  ...
  return (
    alarm && mode && <View style={globalStyles.container}>
